...
module.exports = function (defaultFuncs, api, ctx) {
  ...
  function send(form, threadID, callback, replyToMessage, msg) {
    const effectiveReply = replyToMessage || (msg && (msg.replyToMessage || msg.replyTo));
    if (effectiveReply) {
      form.payload.tasks[0].payload.reply_metadata = {
        reply_source_id: effectiveReply,
        reply_source_type: 1,
        reply_type: 0,
      };
    }

    const textEffectVal = msg && (
      msg.textEffect ||
      msg.text_effect ||
      msg.text_effect_id ||
      msg.textEffectId ||
    ...
    const textEffectId = toTextEffectId(textEffectVal);
    if (textEffectId) {
      form.payload.tasks[0].payload.text_effect_id = textEffectId;
    }
