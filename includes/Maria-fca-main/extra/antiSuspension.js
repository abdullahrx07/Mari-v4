...
class AntiSuspension {
    constructor() {
        ...
        if (this._dailyResetInterval && typeof this._dailyResetInterval.unref === 'function') {
            this._dailyResetInterval.unref();
        }
        if (this._hourlyResetInterval && typeof this._hourlyResetInterval.unref === 'function') {
    ...
    getHumanizedHeaders() {
        ...
        const ua = fp ? { userAgent: fp.ua, secChUa: fp.secChUa, secChUaPlatform: fp.platform } : this._randomUserAgent();
        return {
    ...
    rotateUserAgent() {
        ...
        return this._randomUserAgent().userAgent;
    }

    /**
     * Small built-in user-agent pool so this module has no external
     * dependency on a user-agents.js file that doesn't exist in this repo.
     */
    _randomUserAgent() {
        const pool = [
            {
            ...
            },
            {
                ...
                secChUa: '"Not.A/Brand";v="24", "Chromium";v="125"',
    ...
    checkAccountHealth(lastError) {
        ...
        if (typeof lastError === 'string') {
            errStr = lastError.toLowerCase();
        } else if (lastError && typeof lastError === 'object') {
            errStr = ((lastError.message || '') + ' ' + (lastError.error || '') + ' ' + (lastError.error_summary || '')).toLowerCase();
        }

        const isSuspected = errStr &&
            SUSPENSION_SIGNALS.some(indicator => errStr.includes(indicator));
    ...
    async prepareBeforeMessage(threadID, message) {
        ...
        if (volumeWarning) {
            log.warn("AntiSuspension", volumeWarning);
...
/**
 * This is a standalone utility, not an api endpoint, so it lives in extra/
 * instead of plugins/ and is NOT auto-loaded by buildAPI()'s plugins/ loop.
 * plugins/OldMessage.js and plugins/sendMessage.js pull globalAntiSuspension in
 * directly via require('../extra/antiSuspension').
 * It still exports a function(defaultFuncs, api, ctx) for consistency with
 * the rest of the internal call pattern, but that's a style choice now,
 * not a requirement.
 */
const exportFn = function (defaultFuncs, api, ctx) {
    return {
        AntiSuspension,
...
exportFn.AntiSuspension = AntiSuspension;
exportFn.globalAntiSuspension = globalAntiSuspension;
exportFn.SUSPENSION_SIGNALS = SUSPENSION_SIGNALS;
