...
const commands = {
    ...
    effect: {
        desc: 'Send a message with text effect (hearts, gift, fire, confetti, sparkles)',
        usage: '!effect [type] [text]',
        run: async (api, event, args) => {
            const effectType = args[0] || 'hearts';
            const text = args.slice(1).join(' ') || `Testing ${effectType} text effect! ✨`;
            await api.sendMessage(
                { body: text, textEffect: effectType },
                event.threadID,
                null,
                event.messageID
            );
