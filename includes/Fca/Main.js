...
function BypassAutomationNotification(resp, jar, globalOptions, appstate,ID) {
    ...
    try {
        let UID;
        if (ID) UID = ID
        else if (appstate && typeof appstate.find === "function") {
            const Found = appstate.find(i => i.key == 'c_user') || appstate.find(i => i.key == 'i_user');
            UID = Found ? Found.value : undefined;
        }
        else {
            // appstate not available (fresh email/password login) -> derive UID from jar cookies
            try {
                const cookies = jar.getCookies("https://www.facebook.com");
                const Found = cookies.find(c => c.cookieString().split("=")[0] === "c_user")
                    || cookies.find(c => c.cookieString().split("=")[0] === "i_user");
                UID = Found ? Found.cookieString().split("=")[1] : undefined;
            }
            catch (e) {
                UID = undefined;
            }
...
function makeLogin(jar, email, password, loginOptions, callback, prCallback) {
    return function(res) {
        ...
        html.split("\"_js_").slice(1).map((val) => {
            // FIX: previously, if a "_js_" segment's extracted content
            // wasn't valid JSON (Facebook's page markup for these inline
            // cookie-setting scripts has changed/varies), JSON.parse()
            // threw a SyntaxError synchronously here. Since this runs
            // inside the .then(makeLogin(...)) handler, that throw
            // rejected the ENTIRE login promise chain with a generic
            // "Expected ',' or ']' ..." SyntaxError, aborting the login
            // before the actual email/password POST request was even
            // sent. Skip segments that don't parse instead of aborting.
            try {
                jar.setCookie(utils.formatCookie(JSON.parse("[\"" + utils.getFrom(val, "", "]") + "]"), "facebook"),"https://www.facebook.com")
            } catch (e) {
                // ignore malformed/irrelevant "_js_" segment and continue
            }
        });
