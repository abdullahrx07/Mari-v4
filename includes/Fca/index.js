...
module.exports = function(loginData, options, callback) {
    ...
    // NOTE: an auto-update/auto-install-Node block used to live here
    // (fetching "InstantAction.json" from a third-party GitHub repo and,
    // depending on its contents, silently downloading and running a Node.js
    // installer with `sudo cp -R * /usr/local/`). It was already dead code
    // (commented out upstream), but code that downloads and executes
    // unsigned binaries based on a remote, attacker-controllable JSON file
    // has no place in a login library, so it has been deleted rather than
    // left on disk as an inert landmine. Login proceeds directly below.
    try {
