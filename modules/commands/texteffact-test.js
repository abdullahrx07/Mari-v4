...
module.exports.run = async function ({ api, event, args }) {
	...
	// NOTE: text effects only render when the message goes out over MQTT
	// (api.listenMqtt must already be running — Mari-v4's main.js starts
	// this at login, so it should normally be connected by the time any
	// command runs). If you ever see the effect silently not appear, check
	// the console for a "text effect ... will be lost" warning from
	// sendMessage.js — that means MQTT wasn't connected and it fell back
	// to the legacy HTTP endpoint, which does not support effects.
	return api.sendMessage(
		{
			body: messageText,
