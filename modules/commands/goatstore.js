...
// Handles bots that name their folders differently: cmds, commands, command,
// plugin(s), script(s), event(s) — either at the project root or nested
// under a wrapper folder like modules/, src/, app/, etc.
const CMDS_DIR_CANDIDATES = [
  "modules/cmds", "modules/commands", "modules/command",
  "modules/plugins", "modules/plugin",
  "modules/scripts", "modules/script",
  "script/cmds", "script/commands", "scripts/cmds", "scripts/commands",
  "src/cmds", "src/commands", "src/scripts",
  "app/cmds", "app/commands", "lib/cmds", "lib/commands",
  "cmds", "commands", "command", "plugins", "plugin", "scripts", "script"
];
const EVENTS_DIR_CANDIDATES = [
  "modules/events", "modules/event",
  "script/events", "script/event", "scripts/events", "scripts/event",
  "src/events", "src/event",
  "app/events", "lib/events",
  "events", "event"
];
const CMDS_NAME_PATTERNS = ["cmds", "commands", "command", "plugins", "plugin", "scripts", "script"];
...
// Fast path: check a fixed list of likely relative paths.
function resolveKnownDir(candidates) {
  const baseDir = process.cwd();
  for (const rel of candidates) {
    const full = path.join(baseDir, ...rel.split("/"));
    try {
...
function hasJsFiles(dir) {
  try { return fs.readdirSync(dir).some(f => f.endsWith(".js")); }
  catch (_) { return false; }
}

// Fallback: breadth-first scan (bounded depth) for a folder whose name
// matches one of the known patterns, in case the project uses an
// unlisted wrapper folder (e.g. "app/cmds", "lib/scripts", "script/cmds", etc.).
// A name match is only accepted immediately if it actually contains .js
// files — otherwise scanning continues deeper (handles nesting like
// "script/cmds" where the outer "script" folder is itself just a wrapper
// and the real command files live one level further in).
function scanForDir(namePatterns, maxDepth = 4) {
  const baseDir = process.cwd();
  ...
  while (queue.length) {
    ...
    for (const ent of entries) {
      ...
      if (namePatterns.includes(lower)) {
        if (hasJsFiles(full)) return full;
...
function detectDir(candidates, namePatterns, fallbackRel) {
  const found = resolveKnownDir(candidates) || scanForDir(namePatterns);
  return found || path.join(process.cwd(), ...fallbackRel.split("/"));
}

// FIX 1: cached path is only trusted if it still exists AND still actually
// contains .js files. A stale/empty cached path (e.g. after moving/renaming
// the real cmds folder) will no longer be blindly reused — it forces a
// fresh detection instead. Events dir intentionally skips the hasJsFiles
// check since a fresh/empty events folder is legitimately valid.
function getCmdsDir(forceRescan = false) {
  if (!forceRescan && _dirCache.cmdsDir && fs.existsSync(_dirCache.cmdsDir) && hasJsFiles(_dirCache.cmdsDir)) {
    return _dirCache.cmdsDir;
  }
  const dir = detectDir(CMDS_DIR_CANDIDATES, CMDS_NAME_PATTERNS, "modules/cmds");
...
function getEventsDir(forceRescan = false) {
  ...
  const dir = detectDir(EVENTS_DIR_CANDIDATES, EVENTS_NAME_PATTERNS, "modules/events");
  _dirCache.eventsDir = dir;
  saveDirCache(_dirCache);
  return dir;
}

function resetDirCache() {
  _dirCache = {};
  saveDirCache(_dirCache);
}
...
// FIX 2: force a fresh directory detection at install time instead of
// trusting whatever was last cached in goatstore_dircache.json. This makes
// "gs install <id>" always land in the currently-detected cmds/events
// folder, even if the cache was stale or pointed at an old/moved path.
async function doInstall(api, threadID, id, forceKind = null) {
  ...
  const baseDir = process.cwd();
  const installDir = isEvent ? getEventsDir(true) : getCmdsDir(true);
...
// Scans the detected commands folder, keeps only files whose author matches
// TRACKED_AUTHOR, and returns those where the store has a strictly newer version.
async function checkCommandUpdates() {
  const files = getLocalCommandFiles();
  const baseDir = getCmdsDir();
  const results = [];
  ...
  config: {
    ...
    guide: {
      en:
        ...
        "{pn} dirs — Show detected cmds/events folders\n" +
        "{pn} dirs reset — Force re-detect cmds/events folders\n" +
  ...
  onStart: async function ({ api, event, args, threadData }) {
    ...
    if (sub === "dirs") {
      ...
      if (action === "reset") {
        ...
        return api.sendMessage(
          `♻️ Folder detection reset & re-scanned.\n` +
          `├‣ Commands : ${path.relative(process.cwd(), cmdsDir)}\n` +
          `├‣ Events   : ${path.relative(process.cwd(), eventsDir)}\n` +
          `╰────────────◊`,
          threadID
        );
      }
      const cmdsDir = getCmdsDir();
      const eventsDir = getEventsDir();
      return api.sendMessage(
        `📂 Detected Folders\n` +
        `├‣ Commands : ${path.relative(process.cwd(), cmdsDir)} ${fs.existsSync(cmdsDir) ? "✅" : "⚠️ (missing, will be created on install)"}\n` +
        `├‣ Events   : ${path.relative(process.cwd(), eventsDir)} ${fs.existsSync(eventsDir) ? "✅" : "⚠️ (missing, will be created on install)"}\n` +
        `╰────────────◊\n` +
        `💡 Use "${prefix}gs dirs reset" to re-scan if you moved/renamed folders.`,
        threadID
      );
    }
