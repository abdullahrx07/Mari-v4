...
async function checkSelfUpdate() {
  ...
  try {
    ...
    const match =
      cmds.find(c => c.name?.toLowerCase() === "miraistore" && c.author === myAuthor && String(c.type || "").startsWith("mirai")) ||
      cmds.find(c => c.name?.toLowerCase() === "miraistore" && String(c.type || "").startsWith("mirai"));
...
// Scans commands/*.js, keeps only files that are actually Mirai-style AND
// whose author matches TRACKED_AUTHOR, and returns those where the store
// has a strictly newer version.
//
// FIX: previously this only checked the `credits`/`author` regex match
// without verifying the file was actually a Mirai command. That meant a
// Goat-style command file could accidentally get picked up (e.g. if its
// text happened to contain a matching credits-like string) and get pulled
// into the mirai update-check / autoupdate flow. Now we run
// detectFrameworkLocal() first and skip anything that isn't "mirai".
async function checkCommandUpdates() {
  ...
  for (const file of files) {
    ...
    // Only consider genuine Mirai-format command files.
    if (detectFrameworkLocal(content) !== "mirai") continue;
    ...
    try {
      ...
      const match = cmds.find(c =>
        ...
        String(c.type || "").startsWith("mirai")
      );
      if (!match) continue;
