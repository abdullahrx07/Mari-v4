...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
	return async function (event, message) {
		...
		const { body, messageID, threadID } = event; // isGroup is derived below with E2EE awareness
		...
		// ══════════ E2EE THREAD DETECTION ══════════
		// E2EE (end-to-end encrypted) threads use a string threadID containing "@"
		// instead of the usual numeric threadID. These threads don't have normal
		// thread/user data available the same way, so we fall back to safe defaults
		// and let execution continue through the normal handlers below.
		const isE2EEThread = typeof threadID === "string" && threadID.includes("@");
		// ══════════ E2EE GROUP DETECTION ══════════
		// event.isGroup is often false/undefined for E2EE threads even if it's a group.
		// Fall back to participantIDs count if isGroup is not set by FCA.
		const isGroup = event.isGroup || (isE2EEThread && ((event.participantIDs || []).length > 2));
		// ══════════════════════════════════════════
		// ═════════════════════════════════════════
		...
		// ══════════ E2EE FALLBACK DEFAULTS ══════════
		// If this is an E2EE thread and we don't have real thread/user data,
		// set safe defaults so all downstream handlers (onStart, onChat, etc.)
		// don't crash on missing threadData.data / adminIDs / banned, etc.
		if (isE2EEThread) {
			if (!threadData)
		...
		// E2EE threads always use the global config prefix since getPrefix(threadID)
		// relies on numeric thread lookups that don't apply here.
		...
		async function onStart() {
			...
			if (isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, langCode))
				...
				if (!hideNotiMessage.commandNotFound && (!commandName || commandName.trim() === ""))
					{
						let _prefixMsg;
						try { _prefixMsg = utils.getText({ lang: langCode, head: "handlerEvents" }, "prefixOnly", prefix); } catch (_) {}
						if (!_prefixMsg || _prefixMsg.includes("Can't find text")) _prefixMsg = `✍️ Prefix detected! Use ${prefix}help to see commands.`;
						return await message.reply(_prefixMsg);
					}
